...
struct lgw_pkt_rx_s {
    ...
    uint8_t     rf_chain;       /*!> through which RF chain射频链路 the packet was received */
...
struct lgw_pkt_tx_s {
    ...
    uint8_t     bandwidth;      /*!> modulation bandwidth (LoRa only) */
    uint32_t    datarate;       /*!> TX datarate (baudrate波特率 for FSK频移键控, SF信号频率 for LoRa) */
    uint8_t     coderate;       /*!> error-correcting code of the packet (LoRa only) */
